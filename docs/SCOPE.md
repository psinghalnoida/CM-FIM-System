...
| M22 ✅ | **Document Repository + Viewer** | Org-wide document list — deliberately scoped to VEHICLE-linked documents only, matching the design's own mock data — with KPI tiles (Valid/Expiring/Expired; "Missing" dropped, not a computable status without a per-vehicle required-document-types policy nobody's defined) and an averaged OCR confidence column; Document Viewer restyled to the design (confidence bar, Verify/Flag, and "Request re-upload" which reveals the real M5 re-upload flow — no notification mechanism exists to page a specific person). Extends M5/M11, no new models. See [`docs/DOCUMENTS.md`](DOCUMENTS.md). | M16 |
| M23 ✅ | **TAT Dashboard** | Live board of every in-progress case's TAT status (`IN_PROGRESS`/`ON_HOLD` stages only), across incidents and claims. New aggregation query over M8's `CaseStageInstance`, no new models. See [`docs/TAT.md`](TAT.md). | M16 |
| M24 ✅ | **MIS Reports** | Claim ageing (reuses M9's exact bucket definition), TAT compliance %, incident-type frequency, repair turnaround by depot. New aggregation queries, no new models. See [`docs/DASHBOARDS.md`](DASHBOARDS.md). | M16 |
| M25 ✅ | **Fleet Dashboard** | Fleet-wide KPIs + filterable vehicle list (status, open incidents/claims). New aggregation query, no new models. See [`docs/DASHBOARDS.md`](DASHBOARDS.md). | M16 |
| M26 ✅ | **My Work** | Personalized "needs your action" view, scoped to the caller's role — reuses each module's own `WRITE_ROLES`/status definitions rather than inventing a new per-user assignment concept (none exists in the schema). New query, no new models. See [`docs/DASHBOARDS.md`](DASHBOARDS.md). | M16, M18 |
| M27 ✅ | **Administration: Master Data** | Turns free-text `surveyorName`/`workshopName`/insurer names into real, manageable master entities (Insurer/Broker/Surveyor/Workshop). Real schema migration, one hand-written file (structural changes + a real backfill of every existing free-text value into a matching master row + hard cutover, all in one transaction — safe since this system has no live production data yet). Broker is net-new (nothing referenced one before). Survey/Repair-job creation forms now pick from master data instead of free text. See [`docs/MASTERS.md`](MASTERS.md)'s M27 section. | M16, M18 |
| M28 ✅ | **Vehicle Detail (tabbed profile)** | An 8-tab vehicle page (Information/Status/Documents/Incidents/Claims/Repair History/Warranty/Telematics) — no original design file was available, so the exact tab structure and Warranty's business terms (provider, coverage description, validity window) were confirmed with the user before building rather than guessed at. Telematics tab stays a placeholder pending M12. See [`docs/MASTERS.md`](MASTERS.md)'s M28 section. | M16 |
| M29 ✅ | **Administration: Integration Settings** | Status view of WhatsApp/Telematics/OCR/Email adapters — a real "is this configured and reachable" check reusing `getOcrProvider()`/`getEmailProvider()` directly, not an env-var echo. M10/M12 were never built, so WhatsApp/Telematics report an honest "Not built yet" rather than a fabricated result — confirmed with the user before building. Closes `docs/DEPLOYMENT.md`'s flagged `/api/health` gap with the same underlying check, deliberately unauthenticated for orchestrator probes. See [`docs/INTEGRATIONS.md`](INTEGRATIONS.md). | M16 |
| M30 ✅ | **"Mitra" AI assistant** | Read-only Q&A over live fleet/incident/claim data via a floating chat widget (built from scratch — M16 never actually built the "chat widget in M16's shell" this row assumed, and no design file exists for it). `AssistantProvider` adapter (same pattern as `OCRProvider`/`EmailProvider`) — default unset resolves to a real deterministic stub; `ASSISTANT_PROVIDER=claude` + `ANTHROPIC_API_KEY` is the real thing (official `@anthropic-ai/sdk`), fails closed on an unrecognized provider name. A confirmed "Core 5" tool set (search + get-by-id for incidents/claims/vehicles, plus My Work), each wrapping an existing service-layer query with the asking user's real `AuthSession` — so `scopedDb()`/RBAC apply exactly as everywhere else, no parallel access model. Never writes anything; conversation history stays client-side/ephemeral, not persisted. Usage logging and rate-limiting deliberately left as open questions, not built speculatively. See [`docs/MITRA.md`](MITRA.md). | M16 |
